class mysql 
{
    ...
    { 
        "create-hantim-db":
            unless => "/usr/bin/mysql -uroot -p$mysqlPassword hantim",
            command => "/usr/bin/mysql -uroot -p$mysqlPassword -e 'create database `hantim`;'",
            require => [Service["mysql"], Exec["set-mysql-password"]]
    }
    ...
    { 
        "grant-hantim-db":
            command => "/usr/bin/mysql -uroot -p$mysqlPassword -e 'grant all on `hantim`.* to `root@localhost`;'",
            require => [Service["mysql"], Exec["create-hantim-db"]]
    }
}
